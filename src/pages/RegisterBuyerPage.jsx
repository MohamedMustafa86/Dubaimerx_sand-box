...
const RegisterBuyerPage = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const registrationData = { ...formData, userType: 'buyer', registeredAt: new Date().toISOString(), id: Date.now().toString() };
    localStorage.setItem('dubaimerx_user_buyer', JSON.stringify(registrationData));
    toast({
      title: "Registration Successful! 🎉",
      description: "Welcome to DUBAIMERX! Your buyer account has been created.",
      duration: 5000,
    });
  ...
  return (
    <>
      <Helmet>
        <title>{t('buyerRegistration')} - DUBAIMERX</title>
      </Helmet>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <Card className="shadow-2xl border">
              <CardHeader className="text-center">
                <CardTitle className="text-2xl font-bold">{t('buyerRegistration')}</CardTitle>
                <CardDescription>{t('registerAsBuyerDesc')}</CardDescription>
              </CardHeader>
              <CardContent>
